...
order: 7
legacyUrl: /tutorials/ZZZRendering/XingJianYa(Miyabi)/UnityPJ/08_颜色锐化处理/
draft: false
---
...
![](/assets/img/ZZZRendering/Pasted%20image%2020250315135348.png)
***pixelNDotL，含贴图法线发兰伯特拥有很多细节***
![](/assets/img/ZZZRendering/Pasted%20image%2020250314212626.png)
***NDotL，顶点法线的兰伯特非常光滑细节较少***
![](/assets/img/ZZZRendering/Pasted%20image%2020250314212553.png)

***NDotL - pixelNDotL，两者相减可以得到物体的凹凸信息，那么用1减去这个值就可以得到一个遮罩的遮蔽信息occlusion***
![](/assets/img/ZZZRendering/Pasted%20image%2020250314212910.png)

### 遮蔽信息调整

![](/assets/img/ZZZRendering/Pasted%20image%2020250315140300.png)
***1 -  (NDotL - pixelNDotL)***
![](/assets/img/ZZZRendering/Pasted%20image%2020250315140809.png)
***当前遮蔽信息还是不够强烈所以先乘上3在用1减去，这样增强效果，这里乘以几都可以，可以随意调整，以效果为主,最后进行钳制防止有小于零的值***
***saturate((1 - 3 * (NDotL - pixelNDotL)))***
![](/assets/img/ZZZRendering/Pasted%20image%2020250315141344.png)
***occlusion的杂色太多了，所以整体提亮2倍，去除一些杂色，只保留较小的信息，这里乘几同样也是经验值类的，自己随意调整以效果为准***
***(saturate((1 - 3 * (NDotL - pixelNDotL))) * 2***
![](/assets/img/ZZZRendering/Pasted%20image%2020250315141505.png)
***这里进行了occlusion乘上他的平方根，这样写不太好理解作用，它等价于pow(occlusion，1.5)。那么pow的作用就增加对比度，亮度更亮暗的更暗，最后在钳制防止超过1***
![](/assets/img/ZZZRendering/Pasted%20image%2020250315142125.png)
...
![](/assets/img/ZZZRendering/Pasted%20image%2020250315142618.png)
...
![](/assets/img/ZZZRendering/Pasted%20image%2020250315143736.png)
***这里函数内部对颜色各个分量取得最大值，如果大于1，就将整个颜色除以这个值进行压暗，保证颜色不变的情况下降低明度，钳制到1以下***
![](/assets/img/ZZZRendering/Pasted%20image%2020250315143844.png)

#### 计算颜色亮度和gamma值

***这里Luminance()函数是根据权重计算颜色的灰度（亮度）值。最终通过灰度值来控制Gmma值基数，使用颜色Gamma矫正衰减系数混合，这里这个乘 0.2875和加1.4375的操作也属于经验值，并不是固定的，以效果为准***
![](/assets/img/ZZZRendering/Pasted%20image%2020250315144115.png)

***Luminance()函数在UnityCG.cginc中***
***Unity的颜色权重宏定义***
![](/assets/img/ZZZRendering/Pasted%20image%2020250315144555.png)
***Luminance()函数原型***
![](/assets/img/ZZZRendering/Pasted%20image%2020250314221353.png)
***默认的SRGB权重***
![](/assets/img/ZZZRendering/Pasted%20image%2020250315144954.png)
***现在有了Gama值后，现在就进行颜色的Gamma矫正了。直接使用pow即可，钳制一下不要等于0了，根据效果来调整强度，使用兰伯特控制，亮的部分就使用Gamma，暗的地方就减小Gamma强使用GammaHalf***
![](/assets/img/ZZZRendering/Pasted%20image%2020250315145551.png)
***完整代码***
![](/assets/img/ZZZRendering/Pasted%20image%2020250315153921.png)
...
***左边是无处理，右边是映射了的***
***可以看出处理过之后颜色变化更明显了***
![](/assets/img/ZZZRendering/Pasted%20image%2020250315152926.png)
