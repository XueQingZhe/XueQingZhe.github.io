...
test('real publisher: reconcile, edit existing content, create collection, import work, build, upload and rescan', {timeout:300000},async t=>{
  ...
  assert.deepEqual(request.postDataJSON(),{metadata:{title:'测试作品合集',summary:'独立封面与有序文章组成的专题目录。',cover:'/workflow-cover.png',coverVideo:'',tags:[],engine:[],role:[],category:'',year:new Date().getFullYear(),featured:false},notes:['published:'+work.slug,'notes:workflow-existing']});
  ...
  const writeFromDock=async()=>{
    assert.equal(await page.locator('#nextActionButton').getAttribute('data-target'),'analyze');
    await page.locator('#nextActionButton').click();await idle(page);
    if(await page.locator('#assetApprove').isVisible())await page.locator('#assetApprove').click();
    assert.equal(await page.locator('#nextActionButton').getAttribute('data-target'),'prepare');
    await page.locator('#nextActionButton').click();await idle(page);
    await page.locator('#nextActionButton').click();assert.equal(await page.locator('#applyReview').isVisible(),true);
    await page.locator('#nextActionButton').click();await page.waitForFunction(()=>document.querySelector('main').getAttribute('aria-busy')!=='true'&&document.querySelector('#applyStatus').textContent.includes('已更新'),null,{timeout:120000});
  };
  await page.locator('[data-content-visibility="notes:workflow-existing"]').click();
  assert.match(await page.locator('#visibilityTitles').innerText(),/实际流程：系列文章/);
  await page.locator('#visibilityConfirm').click();await idle(page);
  assert.match(await page.locator('[data-site-key="notes:workflow-existing"]').innerText(),/待移除|撤销移除/);
  await writeFromDock();
  assert.equal(await fs.stat(path.join(site,'dist/notes/workflow-existing/index.html')).catch(()=>null),null,'The real HTTP write removes the article route');
  assert.doesNotMatch(await read('notes'),/href="\/notes\/workflow-existing\/"/);
  assert.doesNotMatch(await read('work/'+collection.id),/href="\/notes\/workflow-existing\/"/);
  assert.equal(await page.locator('[data-site-key="notes:workflow-existing"]').count(),0,'An applied removal moves out of the default inventory');
  await page.locator('#siteStatus').selectOption('withdrawn');
  await page.locator('[data-content-visibility="notes:workflow-existing"]').click();
  await page.locator('#visibilityConfirm').click();await idle(page);
  await writeFromDock();
  assert.match(await read('notes/workflow-existing'),/实际流程：系列文章/);
  const restoredGroup=await read('work/'+collection.id);
  const restoredMembers=await page.evaluate(html=>[...new DOMParser().parseFromString(html,'text/html').querySelectorAll('[data-collection-member] > a')].map(link=>link.getAttribute('href')),restoredGroup);
  assert.deepEqual(restoredMembers,['/notes/'+work.slug+'/','/notes/workflow-existing/']);
