...
作品需要封面和摘要，可点选引擎、职责并填写年份。封面点击“选择图片或视频”打开缩略图库，默认列出本篇引用，也可切换到全部素材；支持按文件名 / 目录搜索、图片 / 视频筛选及分页，同名文件会显示完整路径。点击素材会自动填写路径并显示预览，也保留手动粘贴图片外链的入口。

视频可作为作品封面：自动提取静态封面，桌面卡片悬停 / 键盘聚焦时静音预览，详情页可手动播放并打开声音；手机与减少动态模式使用静态卡片。支持 MP4、WebM、MOV、M4V，后两种会设置为转换 MP4，转换和提取静态封面需要本机 FFmpeg。视频与图片都进入原有的附件审核和压缩流程，只有应用单篇设置后才保存这次选择；原文件保留。浏览素材不会上传文件，也不改变正文。

批量设置仅作用于当前筛选中已选中的笔记，支持设置入口、子栏目、系列以及追加 / 替换 / 移除 Tag。未勾选的子栏目和系列保持原值，选择“不设”才会清空；跨入口批量设置时先选统一入口，切换入口会清空旧的子栏目和系列选择。
