...
每篇笔记的“设置”可编辑网站标题、栏目、日期、Tag、摘要、系列与排序。栏目包括渲染手记、学习系列、作品与实践；作品还需要封面和摘要，可填写引擎、职责、年份。本地封面与正文图片一起审核和压缩。批量设置仅作用于当前筛选中已选中的笔记，支持设置栏目、追加 / 替换 / 移除 Tag 及设置系列。Tag 用逗号分隔，`Unreal Engine` 这样的含空格标签作为一项保留。
