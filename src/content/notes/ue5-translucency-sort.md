...
***在下面声明获取函数，类型为int32***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250420113129.png)

## MaterialInterface.cpp

***在CPP文件内进行函数实现，因为没有函数成员，所以返回0就行***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250420113528.png)

## Material.h

***UMaterial继承自MaterialInterface，在这里进行数据存储，声明变量成员***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250420113808.png)

## MaterialInstance.h

***与UMaterial同理，UMaterialInstance继承自MaterialInterface，进行变量声明***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250420114047.png)

## Material.h

***UMaterial继承自MaterialInterface，所以需要对虚函数进行重写。这里进行声明***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250414151544.png)

## Material.cpp

***CPP文件进行函数实现***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250414152047.png)

## MaterialInstance.h

***与UMaterial同理，UMaterialInstance继承自MaterialInterface，所以需要对虚函数进行重写。这里进行函数声明***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250414152608.png)

## MaterialInstance.cpp

***CPP文件进行函数实现***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250414154844.png)

## MaterialInstanceBasePropertyOverrides.h

***这里是负责比对数据的类，这里声明比对对应的bool。因为是bool，这里变量类型是uint8***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250414155429.png)
***作为数据比对的类，自然也需要有对应变量，所以在下面继续添加相关变量。注意这里变量类型是int32***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250414155604.png)

## MaterialInstance.cpp

***对材质实例的判断是否覆写的函数修改，UMaterialInstance和父类UMaterialInterface数据比较来判断是否重写数据***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415110000.png)

## MaterialShared.cpp

***初始化结构体中的bool参数和属性默认值，这里需要注意顺序，之前MaterialInstanceBasePropertyOverrides中声明位置顺序需要与这里对应***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250420161019.png)
***重写等等操作符的时候需要追加判断，上面是判断Bool是否相等，而下面是判断值是否相对***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250419132240.png)

## MaterialInstance.cpp

***设置UMaterialInstance的更新函数中MaterialTranslucencySortPriority和MaterialTranslucencySectionPriority的初始值***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415111738.png)
***在初始化下面一点设置更新逻辑***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415111854.png)

## PreviewMaterial.cpp

***这里对应的是参数面板中的材质预览使用的材质的处理。材质预览材质对内部材质实例之间的数据拷贝，预览材质直接使用材质实例的参数***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415112506.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415131808.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415133010.png)
***对应的宏内容是这个，其实就是封装的函数体***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415160642.png)
***对于函数体这里还有一点，这里是使用的编辑器对象的内容去判断是否发生Changed操作***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415160610.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415164201.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415161804.png)
***创建UI以及UI的执行逻辑***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415133204.png)
***对应的宏内容是这样的***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415154137.png)
***里面有两个Lambda判断操作，第一个就是是否显示重置按钮，也就是下面编辑面板这个图标。第二Lambda则是对应点击这个重置按钮会执行什么操作，其实就是从父类直接拿默认值数据过来***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415150816.png)
***这里代码根本上是在调用下面这个函数，第一个是参数名称，第二个和第三个是不是很眼熟，其实就是前面创建过函数作为UI参数。三和四就是上面说的Lambda判断，最后一个则是控制是否显示当前UI，默认是true***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415154634.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415170448.png)
***对应就是材质面版里的各种显示设置***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415171147.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415172426.png)
******找到FMaterialResource类，它继承FMaterial类，负责真正的数据处理***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250403232649.png)
***声明继承过来的需要重写的虚函数***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415172812.png)

## MaterialShared.cpp

***对虚函数进行实现，作比对看是返回UMaterial还是UMaterialInstance的值去渲染***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415174511.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250415180940.png)
***对应这里类型转换的计算方法，直接照抄原本的就行***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250408111606.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416113649.png)
***因为需要做排序，所以方便起见直接声明一个结构体来方便使用，当然直接声明在PrimitiveSceneProxy.h和SkeletalMeshSceneProxy.h中也可以，只不过需要声明两次。***
***找到这个类***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416122502.png)
***将结构体丢在上面一点就行，不要放在类里面***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416124100.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416133650.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416133747.png)

## StaticMeshSceneProxy.h

***先处理静态网格体数据。因为继承关系，现在进行声明需要重写负责排序的虚函数***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416133129.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416134229.png)
***根据流程，需找到获取Mesh元素的函数***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416135451.png)
***Mesh元素在GetDynamicMeshElements这里处理***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416135614.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416135217.png)
***对应MeshIdInPrimitive位置如下***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416135249.png)

## StaticMeshSceneProxy.h

***为了将我们排序后的索引给到MeshIdInPrimitive，直接对GetMeshElement函数进行重载，这里进行函数声明，将排序好的索引传入。***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416140315.png)

## StaticMeshRender.cpp

***重载的GetMeshElement函数直接调用原本GetMeshElement函数，最后将将我们排序后的索引给到MeshIdInPrimitive就行***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416140710.png)
***再次找到获取动态Mesh元素的函数GetDynamicMeshElements***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250408195557.png)
***找到GetMeshElement位置，替换成重载过的GetMeshElement即可，将排序好的索引传入***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416141359.png)
***到这里GetDynamicMeshElements函数其实已经改完了，但是在函数末尾Collector.AddMesh会将我们设置的MeshIdInPrimitive给覆盖掉，所以需要做额外处理***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416141840.png)

## SceneManagement.cpp

***Collector.AddMesh定义在SceneManagement.cpp中，SceneManagement负责处理FStaticMeshSceneProxy和FSkeletalMeshSceneProxy等相关数据。找到对应函数***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416142215.png)
***在函数内部可以找到，这里把我们设置的MeshIdInPrimitive给覆盖掉了***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416142247.png)

## SceneManagement.h

***为了解决值覆盖，这里对AddMesh进行重载，新增一个bool来控制是否覆盖***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416142533.png)

## SceneManagement.cpp

***AddMesh重载函数实现直接拷贝一份默认的AddMesh就行，位移修改就是使用bool判断赋值***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416142654.png)

## StaticMeshRender.cpp

***找到GetDynamicMeshElements末尾的AddMesh函数替换成重载过的AddMesh重载函数***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416142833.png)
***最后找到绘制静态元素的地方，这里需要做和GetDynamicMeshElements相似的处理***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250408194422.png)

***索引排序以及调用重载的GetMeshElement函数***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416143526.png)
***再下面一点也需要修改，同样的进行索引排序***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416144013.png)
***以及调用重载的GetMeshElement函数***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416144244.png)
***现在静态网格就修改完毕了***

## SkeletalMeshSceneProxy.h

***骨骼网格体也需要修改，因为继承关系，对排序函数进行重写声明***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416145051.png)

## SkeletalMesh.cpp

***排序函数实现***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416150746.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416153152.png)

## SkeletalMeshSceneProxy.h

***直接修改原函数比较快但没有留下原来的函数，还是不太好，所以最好还是重载一个新GetDynamicElementsSection方法***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416153722.png)

## SkeletalMesh.cpp

***函数实现直接拷贝原来的就好***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416154317.png)
***内部还需要修改，将排序的索引指定给MeshIdInPrimitive***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416154253.png)
***将Collector.AddMesh换成我们重载过的***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416155813.png)
***找到GetDynamicMeshElements函数***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416160036.png)
***它调用了GetDynamicElementsSection，所以需要换成我们重载过的，这里先进行排序索引***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416160403.png)
***替换函数，将排序索引传入***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416160524.png)
***最后找到DrawStaticElements函数进行处理，如果不处理这部分，那么你在骨骼网格体资产预览是对的，但放在场景中却不生效***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416160706.png)
***计算排序索引***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416160836.png)
***将排序好的索引指定给MeshIdInPrimitive***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416160932.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416161822.png)
...
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416224949.png)
***材质上全部都勾选MaterialTranslucencySortPriority，值都为0，状态不变和上图一样***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416224618.png)
***调整红色材质MaterialTranslucencySortPriority值为1，排序发生改变，因为红色数值更大，渲染更靠后，在蓝色之后渲染再叠加***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416224932.png)

### MaterialTranslucencySectionPriority测试

***同样勾选MaterialTranslucencySectionPriority，值默认的0***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416225218.png)
***当前状态***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416225257.png)
***将红色材质MaterialTranslucencySectionPriority值设为1***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416225334.png)
***此时排序发生改变***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416225411.png)

### 对比测试

***这里看似MaterialTranslucencySectionPriority是和MaterialTranslucencySortPriority效果相同，实际上并不一样。这里在追加一个绿色半透明模型放在前方，默认什么都不开的状态如下***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416225840.png)
***先将红色材质MaterialTranslucencySectionPriority值设为1***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416225929.png)
***现在的状态是红色盖住蓝色，但是没有超过绿色***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416230010.png)
***现在红色材质换成改MaterialTranslucencySortPriority值为1***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416230154.png)
***这里是红色直接覆盖了蓝色和绿色***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416230212.png)
...
***默认情况***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416231422.png)
***修改红色材质MaterialTranslucencySortPriority值为1，排序更改生效***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416231443.png)

### MaterialTranslucencySectionPriority测试

***默认情况***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416231532.png)
***修改红色材质MaterialTranslucencySectionPriority值为1，排序更改生效***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416231604.png)

## 对比测试

***增加一个绿色半透明模型放在前面，默认情况如下图***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416231841.png)
***修改红色材质MaterialTranslucencySortPriority值为1，因为不受模型距离排序控制，所以红色会叠在最前方***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416231933.png)
***修改红色材质MaterialTranslucencySectionPriority值为1，红色会叠在蓝色前方，但不会超过其他模型***
![](/assets/img/MyPJ/Engine/Pasted%20image%2020250416232115.png)
