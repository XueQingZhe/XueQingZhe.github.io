...
const fixtures = [
  ...
  ['work:engine', 'UE engine collection', '/work/engine/', { section: 'work', workType: 'collection', cover: '/covers/engine.png', notes: ['notes:shader', 'notes:learning'] }],
...
async function setup(t, { viewport, linked = false } = {}) {
  ...
  const mock = {
    ...
    topics: [{ key: 'engine', title: 'UE engine collection', summary: 'Synthetic article', url: '/work/engine/', notes: ['notes:shader', 'notes:learning'], pending: false }],
  ...
  const topics = () => ({ topics: mock.topics.map(topic => {
    ...
    return { ...topic, title: entry?.metadata.title || topic.title, summary: entry?.metadata.summary || topic.summary };
  }), articles: siteEntries().filter(entry => entry.section !== 'work').map(entry => ({ key: entry.key, title: entry.title, url: entry.url, section: entry.section })), pending: mock.topics.some(topic => topic.pending) });
  const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/collections') {
      const key = 'collections:collection-fixture', metadata = { ...baseMetadata, title: data.title, summary: data.summary, section: 'work', workType: 'collection', cover: '/covers/placeholder.svg', notes: [] };
      mock.site.push({ key, collection: 'collections', id: 'collection-fixture', path: 'src/data/publisher-content.json', metadata, title: data.title, section: 'work', url: '/work/collection-fixture/', active: true, pending: true, notes: [] });
      mock.topics.push({ key, title: data.title, summary: data.summary, notes: [], url: '/work/collection-fixture/', pending: true });
      mock.publication[key] = { state: 'local', label: '尚未上传', reason: '合集只在本地暂存。' };
      return send({ key, id: 'collection-fixture', metadata, pending: true });
    }
    if (url.pathname === '/api/topics' && req.method === 'POST') {
      const index = mock.topics.findIndex(topic => topic.key === data.key);
      if (index < 0) return send({ error: 'Unknown synthetic collection' }, 400);
      mock.topics[index] = { ...mock.topics[index], ...structuredClone(data), pending: true }; return send(topics());
    }
    ...
    if (url.pathname === '/api/site-covers') return send({ items: [], total: 0, hasMore: false, current: null, ffmpeg: false });
...
test('only learning articles expose series, and changing to a work clears its old series', async t => {
  ...
  await page.locator('#noteMetaClose').click(); await edit(page, 'work:engine');
  assert.equal(await page.locator('#seriesField').isVisible(), false);
  assert.equal(await page.locator('#editWorkType').inputValue(), 'collection');
  assert.equal(await page.locator('#editCollectionMembers').isVisible(), true);
  assert.equal(await page.locator('#editCover').evaluate(input => input.required), false);
  assert.deepEqual(errors, []);
});
...
test('creating a collection opens its member editor and preserves member order after save and reload', async t => {
  const { page, mock, errors } = await setup(t);
  await page.locator('#newCollection').click();
  await page.locator('#collectionTitle').fill('UE source modifications');
  await page.locator('#collectionSummary').fill('A group of engine modification articles');
  await page.locator('#collectionCreate').click(); await settled(page);
  assert.deepEqual(calls(mock, 'collections').at(-1).data, { title: 'UE source modifications', summary: 'A group of engine modification articles' });
  assert.equal(await page.locator('#topicEditor').isVisible(), true);
  assert.match(await page.locator('#topicHeading').innerText(), /UE source modifications/);
  for (const key of ['notes:shader', 'notes:learning']) { await page.locator('#topicArticle').selectOption(key); await page.locator('#topicAdd').click(); }
  ...
  assert.deepEqual(calls(mock, 'topics').filter(call => call.method === 'POST').at(-1).data.notes, ['notes:learning', 'notes:shader']);
  assert.equal(await row(page, 'collections:collection-fixture').locator('.publication-badge').getAttribute('data-state'), 'local');
  assert.match(await row(page, 'collections:collection-fixture').innerText(), /合集成员已保存，待写入预览/);
  ...
  assert.deepEqual(await page.locator('#topicNotes [data-article-key]').evaluateAll(rows => rows.map(row => row.dataset.articleKey)), ['notes:learning', 'notes:shader']);
  await edit(page, 'collections:collection-fixture');
  assert.equal(await page.locator('#editSection').isDisabled(), true, 'virtual collections cannot become source-less ordinary articles');
  assert.equal(await page.locator('#editWorkType').isDisabled(), true, 'virtual collections cannot become source-less single works');
  assert.deepEqual(errors, []);
});
...
test('unsaved metadata and member edits block conflicting actions and can be abandoned', async t => {
  ...
  assert.equal(calls(mock, 'content-settings').length, 0);
  assert.equal(calls(mock, 'topics').some(call => call.method === 'POST'), false);
...
test('the dark workbench and collection editor fit a 390px viewport', async t => {
  ...
  await edit(page, 'work:engine'); await page.locator('#editCollectionMembers').click();
  ...
  if (process.env.PUBLISHER_UI_SCREENSHOT_DIR) {
    ...
    await page.locator('#topicCancel').click(); await page.setViewportSize({ width: 1440, height: 1000 });
