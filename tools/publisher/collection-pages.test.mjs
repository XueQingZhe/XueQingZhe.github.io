...
test('collection landing pages lead with ordered members and the study atlas scales through zero, one and seven series',{timeout:300000},async t=>{
  ...
  await page.goto(base+'/tutorials/');assert.equal(await page.locator('[data-series-portal]').count(),4);assert.ok(await page.locator('[data-study-empty]').isVisible());assert.match(await page.locator('[data-series-totals]').innerText(),/00 个系列/);
  ...
  for(const viewport of [{width:1440,height:1000},{width:390,height:844}]){
    await page.setViewportSize(viewport);await page.goto(base+'/tutorials/');assert.equal(await page.locator('[data-series-portal]').count(),4,'The atlas remains four cards as sub-series grow');assert.deepEqual(await page.locator('[data-series-portal] h2').allTextContents(),['我独自升级','Unity','UE','Blender']);assert.equal(await page.locator('[data-study-series]').count(),7);assert.match(await page.locator('[data-series-totals]').innerText(),/07 个系列/);assert.match(await page.locator('[data-series-totals]').innerText(),/08 篇记录/);assert.equal(await page.locator('[data-series-portal="unity"] .portal-count').innerText(),'3篇');assert.equal(await page.locator('[data-series-portal="ue"] .portal-count').innerText(),'1篇');assert.equal(await page.locator('[data-series-portal="blender"] .portal-count').innerText(),'1篇');assert.equal(await page.locator('[data-series-portal="solo"] .portal-count').innerText(),'0篇');assert.equal(await page.locator('[data-study-direction="other"] [data-study-series]').count(),3,'Unrecognised series stay visible below without taking a top-level slot');
    ...
    await page.locator('[data-series-link]').last().click();assert.equal(decodeURIComponent(new URL(page.url()).hash),'#series-'+names[6]);assert.ok(await page.locator('[data-study-series]').last().isVisible());await page.locator('[data-series-portal="blender"]').click();assert.equal(decodeURIComponent(new URL(page.url()).hash),'#study-blender');
