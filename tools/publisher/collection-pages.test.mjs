...
test('collection landing pages lead with ordered members and the study atlas scales through zero, one and seven series',{timeout:300000},async t=>{
  ...
  await page.goto(base+'/tutorials/');assert.equal(await page.locator('[data-series-portal]').count(),4);assert.ok(!(await page.locator('[data-study-empty]').isVisible()));assert.match(await page.locator('[data-series-totals]').innerText(),/00 个系列/);assert.equal(await page.locator('[data-series-portal="solo"] .portal-count').innerText(),'2篇');assert.equal(await page.locator('[data-study-direction="solo"] [data-study-entry]').count(),2,'Loose public articles are included even before any named series exists');
  ...
  await chapter('single-series',names[0],10);await build();await page.goto(base+'/tutorials/');assert.equal(await page.locator('[data-series-portal]').count(),4,'The atlas keeps four stable discipline cards');assert.deepEqual(await page.locator('[data-series-portal] h2').allTextContents(),['我独自升级','Unity','UE','Blender']);assert.equal(await page.locator('[data-study-series]').count(),2);assert.equal(await page.locator('[data-series-portal="unity"] .portal-count').innerText(),'1篇');assert.match(await page.locator('[data-series-portal="unity"] p').innerText(),/1 个细分系列/);
  await chapter('series-first-chapter',names[0],1);
  for(let i=1;i<names.length;i++)await chapter('series-'+i,names[i],i+1,{engine:i===1?['Unity']:[]});
  ...
  for(const viewport of [{width:1440,height:1000},{width:390,height:844}]){
    await page.setViewportSize(viewport);await page.goto(base+'/tutorials/');assert.equal(await page.locator('[data-series-portal]').count(),4,'The atlas remains four cards as sub-series grow');assert.deepEqual(await page.locator('[data-series-portal] h2').allTextContents(),['我独自升级','Unity','UE','Blender']);assert.equal(await page.locator('[data-study-series]').count(),8);assert.match(await page.locator('[data-series-totals]').innerText(),/07 个系列/);assert.match(await page.locator('[data-series-totals]').innerText(),/10 篇记录/);assert.equal(await page.locator('[data-series-portal="unity"] .portal-count').innerText(),'3篇');assert.equal(await page.locator('[data-series-portal="ue"] .portal-count').innerText(),'1篇');assert.equal(await page.locator('[data-series-portal="blender"] .portal-count').innerText(),'1篇');assert.equal(await page.locator('[data-series-portal="solo"] .portal-count').innerText(),'5篇');assert.equal(await page.locator('[data-study-direction="solo"] [data-study-series]').count(),4,'Unclassified series and loose articles share the solo direction');
    ...
    await page.locator('[data-series-link]').filter({hasText:names[6]}).click();assert.equal(decodeURIComponent(new URL(page.url()).hash),'#series-'+names[6]);assert.ok(await page.locator('[data-study-series]').last().isVisible());await page.locator('[data-series-portal="blender"]').click();assert.equal(decodeURIComponent(new URL(page.url()).hash),'#study-blender');
  ...
  t.diagnostic('Verified a stable four-direction atlas with 0/1/7 sub-series, category anchors, engine fallback, full titles at desktop/mobile sizes, ordered collection directory before preserved prose, independent revised covers, member backlinks and gallery hierarchy without duplicate member tiles.');
});
